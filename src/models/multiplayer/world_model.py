...
class SolarisMPModel(nnx.Module):
    ...
    def __init__(
        ...
        self.eps = eps
        self.platform = platform
        # For multiplayer_attn we allow runtime player counts up to the configured
        # embedding capacity. Under concat_c we still expect an effective P=1.
        self.expected_p_size = 1 if multiplayer_method == "concat_c" else num_players
    ...
    def __call__(
        ...
        if self.multiplayer_method == "concat_c":
            assert (
                p == self.expected_p_size
            ), f"Expected P={self.expected_p_size} for multiplayer_method={self.multiplayer_method}, got P={p}"
        else:
            assert (
                1 <= p <= self.expected_p_size
            ), f"Expected 1 <= P <= {self.expected_p_size} for multiplayer_method={self.multiplayer_method}, got P={p}"
